...
class CryptoSystem {
  ...
  resetSymmetricVisualization(algorithm) {
    ...
    if (algorithm === "AES") {
      document.getElementById("encryptionKey").textContent = "ranjeetranjeet12"
      document.getElementById("blockSize").textContent = "16 bytes (AES)"
    } else if (algorithm === "DES") {
      document.getElementById("encryptionKey").textContent = "ranjeet!"
  ...
  resetRSAVisualization() {
    ...
    steps.forEach((step) => {
      const stepElement = document.getElementById(`rsa-step-${step}`)
      const statusElement = document.getElementById(`rsa-status-${step}`)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("🔑 Master Key: ranjeet")
})
